...
public final class UnlinkCommand implements CommandExecutor, TabCompleter {
    ...
    public UnlinkCommand(StormAuthPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (platform == null) {
            plugin.getMessages().send(sender, "usage", "usage", "/unlink <telegram|vk>");
            return true;
        }
        ...
        plugin.getSocialService().unlink(player, platform);
        return true;
    }
